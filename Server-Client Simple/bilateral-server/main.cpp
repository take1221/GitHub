...
void server_func(void)
{
	...
	while(!ch){
		data.fdata=point_x;    /* 共用体によるデータ変換 */
		buff[0]=data.cdata[0];
		buff[1]=data.cdata[1];
		buff[2]=data.cdata[2];
		buff[3]=data.cdata[3];
		...
		WriteSocket(fd,8,buff); /* サーバが8バイト送信*/

		ReadSocket(fd,8,buff); /* サーバが8バイト受信 */
		...
		if(count%5==0 && start != 0 && end != start){
			printf("Rate(Drafter): %4.2f [Hz]\n",(float)count*1000/(end-start));
			printf("Rate(Server) : %4.2f [Hz]\n\n",(float)Ccount*1000/(end-start));
		}
	}
}
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("drafter server"); /*引数の名前のウインドウ作成*/
	...
	tcp_hthread=CreateThread(NULL, 0,
		(LPTHREAD_START_ROUTINE)server_func, NULL,
		0, &tcp_lpThreadId);
